...
if 'DEEPINFRA_API_TOKEN' not in os.environ:
    print("No DeepInfra API key found in environment variables. Will attempt to read from `api_deepinfra_personal.key`.")
    if os.path.exists('api_deepinfra_personal.key'):
        ...
        print("No `api_deepinfra_personal.key` file found. Please create one with your DeepInfra API key or set the `DEEPINFRA_API_TOKEN` variable.")
        exit()
